...
average rental prices in 2015 but as of 2024, they are reaching
equivalent average rental prices as the other boroughs. Manhattan used
to be relatively similar to the other boroughs in 2015, but average
rental prices have climbed to include a big range of prices. Staten
Island average rental prices seem to have changed the least in
comparison to the other 4 boroughs.

Compute the average rental price in each zip code over each month of
2023:
...
```
    ...
    ## `summarise()` has grouped output by 'zip_code', 'month_num', 'year'. You can
    ## override using the `.groups` argument.

![](p8105_hw3_mt3866_files/figure-gfm/unnamed-chunk-10-1.png)<!-- --> In
2023, the Bronx had the lowest average rental prices. The average rental
prices in the Bronx were similar to Staten Island, but Staten Island had
a smaller range of rental prices while it was vast in the Bronx.
Manhattan had the highest average rental prices while Brooklyn had the
second highest.
...
![](p8105_hw3_mt3866_files/figure-gfm/unnamed-chunk-11-1.png)<!-- -->
...
Filter the merged data set and ensure variables make sense
...
Create age distribution visual

``` r
agedist_df = filtered_df |>
select(age, sex, education)|>
  ...
  labs(
    title = "Visualization of Age Distributions",
    x = "Sex",
    y = "Age")
print(agedist_df)
```

![](p8105_hw3_mt3866_files/figure-gfm/unnamed-chunk-15-1.png)<!-- -->
Commentary: When looking at the age range of 60+ in the high school
equivalent category, more females had reached this level of educational
attainment than males. In the younger generation of ages 20-40, more
individuals including both males and females have attained a level of
education that is greater than high school in comparison to education
that is less than high school.

``` r
activity_df = filtered_df|>
  select(seqn, mim_value, age, sex, education)|>
  group_by(seqn, sex, education, age)|>
summarize(activity=sum(mim_value))|>
ggplot(aes(x = age, y = activity, color = sex)) +
  ...
  labs(
    title = "Total Activity by Age, Sex & Education",
    x = "Age",
    y = "Activity")
```
...
![](p8105_hw3_mt3866_files/figure-gfm/unnamed-chunk-16-1.png)<!-- -->

In all three categories of education, there is a general downward trend
between aging and activity. As individuals age, their activity levels
decrease. This applies to both men and women, however the smallest rate
of change is observed in individuals who have obtained an education
level that is more than high school.

``` r
hour_df = filtered_df|>
  mutate(minute_number = as.numeric(str_remove(minutes,"min")))|>
    group_by(minute_number, sex, education)|>
ggplot(aes(x = minute_number, y = mim_value, color = sex)) +
    geom_point (alpha=.5) +
    theme(axis.text.x = element_text(angle=90, hjust=1, vjust=0.5, size = 10)) +
    facet_grid(. ~ education) +
  geom_smooth(se = FALSE) +
  labs(
    title = "Acitivity Per Minute",
    x = "Minutes",
    y = "Activity")
print(hour_df)
```
...
![](p8105_hw3_mt3866_files/figure-gfm/unnamed-chunk-17-1.png)<!-- --> In
this plot, I can see that both men and females tend to exercise the most
mid day in each category. In individuals who obtained education that is
greater than high school, many males are more active towards the end of
the day in comparison to the other education groups.
